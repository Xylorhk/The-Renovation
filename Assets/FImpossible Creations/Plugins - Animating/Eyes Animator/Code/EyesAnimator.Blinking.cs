...
namespace FIMSpace.FEyes
{
    public partial class FEyesAnimator
    {
        ...
        public enum FE_EyesBlinkingMode { Bones, Blendshapes }
        ...
        [Tooltip("Syncing frequency of blinking with random movement preset choosed in 'Tweaking Animation' -> 'Eyes Random Movement Settings' Tab ")]
        ...
        private void CalculateBlinking(int i = 0)
        {
            ...
            if (i >= blinking.Length) return;
            if (blinking[i] == null) return;
            ...
            if (AdditionalEyelidsMotion > 0f)
            {
                if (upper)
                    if (UpEyelidsBlendShapes.Count > 0)
                    {
                        ...
                        lid.AdditionalFactor = Mathf.Lerp(lid.AdditionalFactor, targetFactor, Time.deltaTime * eyesSpeedValue);

                        if (lid.Animated) lid.PreWeight = BlendShapesMesh.GetBlendShapeWeight(lid.ShapeIndex);
                        else lid.PreWeight = 0;

                        float value = Mathf.LerpUnclamped(lid.Open, lid.Closed, lid.AdditionalFactor);
                        value += lid.PreWeight;
